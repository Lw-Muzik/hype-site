...
<main id="top">
  ...
  <section class="relative overflow-hidden pb-[clamp(40px,6vw,80px)] pt-[clamp(48px,8vw,100px)]">
    <Spectrum
      class="pointer-events-none absolute inset-x-0 bottom-0 h-[56%] w-full opacity-50 [mask-image:linear-gradient(to_top,#000_30%,transparent)]"
    />
    <div
      class="pointer-events-none absolute -top-[10%] left-1/2 h-[60vh] w-[70vw] -translate-x-1/2 blur-2xl"
      style="background:radial-gradient(50% 50% at 50% 50%,rgba(255,202,66,.12),transparent 70%),radial-gradient(45% 45% at 65% 40%,rgba(52,193,58,.10),transparent 70%)"
      aria-hidden="true"
    ></div>
    ...
    <div class="wrap relative z-[2] grid items-center gap-[clamp(24px,4vw,56px)] lg:grid-cols-[1.05fr_0.95fr]">
      ...
      <div class="relative min-h-[400px] [perspective:1400px]" aria-hidden="true">
        <!-- Desktop window -->
        <div
          class="overflow-hidden rounded-[18px] border border-line bg-gradient-to-b from-surface to-surface-1 shadow-[0_40px_80px_-30px_rgba(0,0,0,.8)] transition-transform duration-500 [transform:rotateY(-13deg)_rotateX(5deg)]"
        >
        ...
        <!-- Phone -->
        <div
          class="absolute -bottom-[8%] -right-[6%] w-[168px] rounded-[28px] border border-line-2 bg-gradient-to-b from-surface-2 to-surface-1 px-3 pb-4 pt-3 shadow-[0_50px_70px_-30px_rgba(0,0,0,.85)] [transform:rotateY(-13deg)_rotateX(4deg)_translateZ(40px)]"
        >
          <div class="mx-auto mb-3 h-[5px] w-[46px] rounded-full bg-line-2"></div>
          <div
            class="h-[120px] rounded-[14px]"
            style="background:conic-gradient(from 200deg at 70% 30%,#ffca42,#34c13a,#f5b40f,#ffca42)"
          ></div>
  ...
  <section
    id="connect"
    class="py-[clamp(72px,10vw,140px)] [background:linear-gradient(180deg,transparent,rgba(255,202,66,.03),transparent)]"
  >
    <div class="wrap">
      ...
      >
        <div class="rounded-[18px] border border-line bg-gradient-to-b from-surface to-surface-1 p-7 text-center">
        ...
        <div class="flex flex-col justify-center gap-2.5 px-1.5">
          ...
          <div class="relative h-[3px] overflow-hidden rounded-full [background:linear-gradient(90deg,var(--color-line),var(--color-line-2),var(--color-line))]">
        ...
        <div class="rounded-[18px] border border-gold/35 bg-gradient-to-b from-surface to-surface-1 p-7 text-center shadow-[0_30px_60px_-34px_rgba(255,202,66,.3)]">
      ...
      <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {#each [
          ...
          <article
            class="reveal rounded-2xl border border-line bg-surface-1 p-6 transition-transform duration-300 hover:-translate-y-1 hover:border-line-2"
  ...
  <section id="desktop" class="py-[clamp(72px,10vw,140px)]">
    <div class="wrap">
      ...
      <div class="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <!-- wide: system-wide -->
        <article class="reveal group relative overflow-hidden rounded-[18px] border border-line bg-surface-1 p-[30px] transition-transform duration-300 hover:-translate-y-1 hover:border-line-2 md:col-span-2" use:reveal>
        ...
        <!-- EQ -->
        <article class="reveal group rounded-[18px] border border-line bg-surface-1 p-[30px] transition-transform duration-300 hover:-translate-y-1 hover:border-line-2" use:reveal={{ delay: 60 }}>
          <div class="grid h-[52px] w-[52px] place-items-center rounded-[13px] border border-gold/20 bg-gold/10 text-gold">
          ...
          <p class="mt-2.5 text-[14.5px] text-muted">A live response curve over a real-time spectrum, with 12 presets and 37 AutoEq headphone profiles.</p>
        </article>

        <!-- DSP -->
        <article class="reveal group rounded-[18px] border border-line bg-surface-1 p-[30px] transition-transform duration-300 hover:-translate-y-1 hover:border-line-2" use:reveal={{ delay: 120 }}>
          <div class="grid h-[52px] w-[52px] place-items-center rounded-[13px] border border-gold/20 bg-gold/10 text-gold">
          ...
          <p class="mt-2.5 text-[14.5px] text-muted">Parametric EQ, tone, multiband compression, stereo width, crossfeed, 3D surround and room reverb — into a transparent limiter.</p>
        </article>
        ...
        {#each chain as step, i}
          ...
          <li
            class="relative flex min-w-[132px] flex-1 flex-col gap-2 border border-line px-[18px] py-[22px] first:rounded-l-[14px] last:rounded-r-[14px] {last
              ? 'bg-gradient-to-b from-gold/10 to-surface-2'
  ...
  <section id="mobile" class="py-[clamp(72px,10vw,140px)]">
    <div class="wrap grid items-center gap-[clamp(30px,6vw,80px)] lg:grid-cols-[0.9fr_1.1fr]">
      <div class="order-2 grid place-items-center lg:order-1" aria-hidden="true">
        <div
          class="w-[280px] rounded-[34px] border border-line-2 bg-gradient-to-b from-surface-2 to-surface-1 px-4 pb-[22px] pt-4 shadow-[0_50px_80px_-40px_rgba(0,0,0,.9)] [transform:perspective(1600px)_rotateY(12deg)_rotateX(4deg)]"
        >
          <div class="mx-auto mb-3.5 h-[5px] w-[52px] rounded-full bg-line-2"></div>
          <div class="h-[200px] rounded-[16px]" style="background:conic-gradient(from 40deg at 30% 70%,#34c13a,#ffca42,#6ad1a0,#34c13a)"></div>
      ...
      <div class="order-1 lg:order-2">
        ...
        <ul class="reveal mt-7 flex flex-col" use:reveal={{ delay: 80 }}>
          {#each [
            { b: '31-band EQ & presets', s: 'Graphic + parametric EQ, tone and multiband compression — tuned for headphones.' },
            ...
            { b: 'Be a source, or a speaker', s: 'Stream your library to the desktop, or receive a cast from it.' }
          ] as f}
  ...
  <section id="download" class="py-[clamp(72px,10vw,140px)] text-center">
    <div class="wrap">
      <div class="relative overflow-hidden rounded-[28px] border border-line bg-gradient-to-b from-surface-1 to-bg px-6 py-[clamp(48px,8vw,96px)]">
        <Spectrum
